...
@section('content')
    <main class="w-full px-8">
        ...
        <div class="w-60 h-60 rounded-md shadow-md">
            <img src="{{ '/storage/users/' . auth()->user()->picture }}" class="w-60 rounded-md h-60" alt="Profile Picture">
        </div>

        <form action="/user/change/picture" class="mt-4" method="post" enctype="multipart/form-data">
            @csrf
