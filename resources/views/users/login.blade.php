...
<body class="bg-amber-50">
    ...
    <form action="/login" method="POST" class="bg-amber-400 rounded-md shadow-md p-6 text-white">
        @csrf
        <div class=" flex items-center gap-x-4 justify-center px-8">
            <img src="{{ asset('images/lasu.png') }}" alt="" class="w-16 h-16">
            <h1 class="font-bold text-xl text-white">LASU-LMS</h1>
        </div>
        @if (session('message'))
        ...
        <div class="">
            ...
            <input type="text" name="employee_id" placeholder="Employer Id" class="p-3 text-amber-400 block w-full mt-1.5 rounded-md outline-none" id="">
        </div>
        <div class="">
